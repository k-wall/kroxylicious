...
public class VirtualCluster implements ClusterNetworkAddressConfigProvider {
    ...
    public Optional<SslContext> buildDownstreamSslContext() {
        return tls.map(tls -> new DefaultSslContextFactory().buildServerSslContext(tls));
    }

    public Optional<SslContext> buildUpstreamSslContext() {
        return targetCluster.tls().map(tls -> new DefaultSslContextFactory().buildClientSslContext(tls));
    }
