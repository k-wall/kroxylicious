...
1. Run `T` build/test cycle but use an alternative cache location to be sure artefacts are being fetched.  Check the build output, you'll see the
   kroxylicious come from the staging location.
```bash
...
2. Merge release PR.
3. Publish the release notes for release by following the https://github.com/kroxylicious/kroxylicious/releases.  
...
If the release fails verification, use the [deploy_workflow](https://github.com/kroxylicious/kroxylicious/actions/workflows/deploy_release.yml)
