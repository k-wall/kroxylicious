 ...
 * and an {@link Errors} code, without depending on kafka-clients' request/response wrapper classes.
 * <p>
 * Kafka's own per-RPC error-response bodies are hand-written, not generated, so this factory ports
 * that logic across explicitly, one {@link ApiKeys} case at a time, rather than depending on those wrapper
 * classes, which are out of scope for the {@code io.kroxylicious.kafka.*} migration
 * (see kroxylicious/kroxylicious#4748).
 * <p>
 * kafka-clients always synthesises error responses with a throttle time of 0, so this factory does the same
 * rather than taking a throttleTimeMs parameter.
 * <p>
 ...
 * client doesn't want a response at all, mirroring Kafka's own behaviour for that case.
 */
